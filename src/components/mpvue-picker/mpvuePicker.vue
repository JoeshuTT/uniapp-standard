<template>
  <view class="mpvue-picker">
    <view :class="{ pickerMask: showPicker }" catchtouchmove="true" @click="maskClick"></view>
    <view class="mpvue-picker-content" :class="{ 'mpvue-picker-view-show': showPicker }">
      ...
      >
        <block>
          <picker-view-column>
            <view v-for="(item, index) in pickerValueSingleArray" :key="index" class="picker-item">{{
              item.label
            }}</view>
          </picker-view-column>
        </block>
      ...
      >
        <block>
          <picker-view-column>
            <view v-for="(item, index) in pickerValueHour" :key="index" class="picker-item">{{ item.label }}</view>
          </picker-view-column>
          <picker-view-column>
            <view v-for="(item, index) in pickerValueMinute" :key="index" class="picker-item">{{ item.label }}</view>
          </picker-view-column>
        </block>
      </picker-view>
      <!-- 多列选择 -->
      ...
      >
        <block v-for="(n, index) in pickerValueMulArray.length" :key="index">
          <picker-view-column>
            <view v-for="(item, index1) in pickerValueMulArray[n]" :key="index1" class="picker-item">{{
              item.label
            }}</view>
          </picker-view-column>
        </block>
      ...
      >
        <block>
          <picker-view-column>
            <view v-for="(item, index) in pickerValueMulTwoOne" :key="index" class="picker-item">{{ item.label }}</view>
          </picker-view-column>
          <picker-view-column>
            <view v-for="(item, index) in pickerValueMulTwoTwo" :key="index" class="picker-item">{{ item.label }}</view>
          </picker-view-column>
        </block>
      </picker-view>
      <!-- 三级联动 -->
      ...
      >
        <block>
          <picker-view-column>
            <view v-for="(item, index) in pickerValueMulThreeOne" :key="index" class="picker-item">{{
              item.label
            }}</view>
          </picker-view-column>
          <picker-view-column>
            <view v-for="(item, index) in pickerValueMulThreeTwo" :key="index" class="picker-item">{{
              item.label
            }}</view>
          </picker-view-column>
          <picker-view-column>
            <view v-for="(item, index) in pickerValueMulThreeThree" :key="index" class="picker-item">{{
              item.label
            }}</view>
          </picker-view-column>
        </block>
    ...
    pickerChangeMul(e) {
      if (this.deepLength === 2) {
        ...
        if (changeValue[0] !== this.pickerValue[0]) {
          ...
          for (
            let i = 0, length = pickerValueArray[changeValue[0]].children[changeValue[1]].children.length;
            i < length;
            i++
          ) {
